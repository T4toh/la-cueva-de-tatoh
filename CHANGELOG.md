...
- **Las comidas cargadas en otro dispositivo ya no se pisan al refrescarse la sesión.** `saveToFirestore` leía `currentUser()` adentro de los efectos de persistencia, así que después de la primera edición cada efecto dependía de la sesión, y `user()` emite un objeto nuevo en cada refresco del token. El primero llega al volver la PWA del fondo, antes de que la escucha en vivo se ponga al día: el efecto volvía a subir el `meals` viejo de este dispositivo encima de lo que había cargado el otro. Ahora la lectura es `untracked`, igual que en `uidOrThrow` de `RecetaPublicaService`, que corre desde el mismo efecto. Y `currentUser` arranca en `undefined` hasta que auth resuelve: lo que se cargaba en ese rato se descartaba como "sin sesión" y el primer snapshot lo pisaba; ahora queda pendiente y se reenvía, y si resuelve sin sesión se descarta.
